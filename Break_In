...
Trolling:Textbox("Become Stealth For X Seconds", true, function(Text)
game:GetService("ReplicatedStorage").RemoteEvents.GiveTool:FireServer("TeddyBloxpin")
game:GetService("ReplicatedStorage").RemoteEvents.MakeStealth:FireServer(tonumber(Text))
wait(.2)
if game.Players.LocalPlayer.Backpack:FindFirstChild("TeddyBloxpin") then
    game.Players.LocalPlayer.Backpack:FindFirstChild("TeddyBloxpin"):Destroy()
elseif game.Players.LocalPlayer.Character:FindFirstChild("TeddyBloxpin") then
    game.Players.LocalPlayer.Character:FindFirstChild("TeddyBloxpin"):Destroy()
end
end)
...
Character:Toggle("Auto-Heal", function(state)
Client.toggles.Heal = state
end)
...
spawn(function()
    while task.wait() do
    if Client.toggles.Heal and game:GetService("Players").Ihaveash0rtname.PlayerGui.EnergyBar.EnergyBar.EnergyBar.Warning.Visible or game:GetService("Players").Ihaveash0rtname.PlayerGui.EnergyBar.EnergyBar.EnergyBar.Warning2.Visible then
        game:GetService("ReplicatedStorage").RemoteEvents.GiveTool:FireServer("BloxyCola")
        game:GetService("ReplicatedStorage").RemoteEvents.Energy:FireServer(15, "BloxyCola")
    end
end
end)
